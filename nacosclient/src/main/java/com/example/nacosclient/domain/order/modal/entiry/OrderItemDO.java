package com.example.nacosclient.domain.order.modal.entiry;
...
public class OrderItemDO {
    ...
    public OrderItemDO(String itemId, String orderId, BigDecimal price, AddressVObj addressVObj, Integer quantity) {
        // 订单项不能为空
        ParamExceptionEnum.ITEMID_NOT_NULL.assertIsFalse(StringUtils.isNotBlank(itemId));
        // 订单id不能为空
        ParamExceptionEnum.ORDERID_NOT_NULL.assertIsFalse(StringUtils.isNotBlank(orderId));
        // 订单项价格不能小于0
        ParamExceptionEnum.PRICE_NOT_LESS_THEN_ZERO.assertIsFalse(price.compareTo(BigDecimal.ZERO) >= 0);
        // 地址不能为空
        ParamExceptionEnum.ADDRESSVOBJ_NOT_NULL.assertNotNull(addressVObj);
        // 订单项数量要大于0
        ParamExceptionEnum.QUANTITY_NOT_LESS_THEN_ZERO.assertIsFalse(quantity > 0);
